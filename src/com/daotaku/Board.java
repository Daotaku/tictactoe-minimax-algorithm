...
public class Board {
    ...
    public void displayBoard() {
        ...
        String[] verticalBoard = {"A", "B", "C"};

        for (int i = 0; i < Constants.BOARD_SIZE; i++)
        {
            System.out.print(verticalBoard[i]+" ");
            for (int j = 0; j < Constants.BOARD_SIZE; j++)
            {
    ...
    public int getInputUserX(String inputUser) {
        Character input = inputUser.charAt(0);
        return input == 'A' ? 0 : (input == 'B' ? 1 : 2);
    }

    public int getInputUserY(String inputUser) {
        Character input = inputUser.charAt(1);

        return input == '1' ? 0 : (input == '2' ? 1 : 2);
    }

    public boolean isFilledCell(Cell cell) {
        if (this.board[cell.getX()][cell.getY()] == Player.NONE) return false;
        return true;
    }
}
